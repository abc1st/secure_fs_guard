# Secure FS Guard

Система контроля целостности и доверенного состояния файловой системы с активным противодействием несанкционированным изменениям.

## Особенности

- 🔒 **Контроль целостности** — поблочное хеширование файлов (SHA-256, блоки 64 KB)
- 🔄 **Активное противодействие** — автоматическое восстановление при нарушениях
- 🚨 **Обнаружение ransomware** — анализ паттернов массового шифрования
- 📊 **Мониторинг в реальном времени** — inotify + fallback периодическая проверка
- 💾 **Резервное копирование** — автоматическое создание и хранение бэкапов
- 🖥️ **GUI интерфейс** — удобное управление через PySide6

## Архитектура

### Служба (root)
- `config_manager.py` — управление конфигурацией
- `logger.py` — журналирование событий безопасности
- `hash_storage.py` — хранилище эталонных хэшей (SQLite)
- `integrity_engine.py` — движок проверки целостности
- `watcher.py` — мониторинг изменений файлов
- `recovery.py` — восстановление и блокировка
- `auth.py` — управление режимами работы
- `ipc_server.py` — IPC сервер (UNIX socket)
- `main.py` — главный модуль

### GUI (пользователь)
- `gui_main.py` — главное окно
- `ipc_client.py` — клиент для связи с службой
- `views/main_window.py` — статус системы
- `views/settings_view.py` — настройки
- `views/integrity_view.py` — проверка файлов
- `views/logs_view.py` — просмотр логов

## Установка

### Зависимости
```bash
sudo apt install python3 python3-pip inotify-tools

pip install -r requirements.txt
```

### Установка системы
```bash
# Запуск установочного скрипта (копирует файлы и настраивает службу)
sudo bash scripts/install_service.sh

# Копирование файлов (Ручная установка)
sudo mkdir -p /opt/secure_fs_guard
sudo cp -r daemon /opt/secure_fs_guard/
sudo cp -r gui /opt/secure_fs_guard/

# Создание директорий
sudo mkdir -p /etc/secure_fs_guard
sudo mkdir -p /var/lib/secure_fs_guard/storage/backups
sudo mkdir -p /var/log/secure_fs_guard

# Установка прав
sudo chmod 700 /var/lib/secure_fs_guard
sudo chmod 700 /var/log/secure_fs_guard
```

## Использование

### Запуск службы
```bash
# Ручной запуск
sudo python3 /opt/secure_fs_guard/daemon/main.py

# Или через systemd (см. scripts/install_service.sh)
sudo systemctl start secure-fs-guard
sudo systemctl enable secure-fs-guard
```

### Запуск GUI
```bash
python3 /opt/secure_fs_guard/gui/gui_main.py
```

## Конфигурация

Файл: `/etc/secure_fs_guard/system.yaml`
```yaml
protected_paths:
  - /home/*/Documents
  - /home/*/important_data

block_config:
  size: 65536  # 64 KB
  algorithm: sha256

ransomware_thresholds:
  files_count: 10
  time_window: 10
  block_change_percent: 70
  entropy_threshold: 7.5

monitoring:
  fallback_interval: 60
  use_inotify: true
```

## Режимы работы

1. **MONITOR** (контроль) — основной режим, мониторинг и восстановление
2. **INIT** (инициализация) — создание эталонного состояния
3. **UPDATE** (обновление) — временное разрешение изменений
4. **EMERGENCY** (аварийный) — блокировка при обнаружении атаки

## Алгоритм работы

1. Служба запускается под root
2. Загружается конфигурация и эталонные хэши
3. Запускается мониторинг файлов (inotify + fallback)
4. При изменении файла:
   - Вычисляются текущие хэши блоков
   - Сравниваются с эталонными
   - При нарушении — автоматическое восстановление
5. При обнаружении ransomware:
   - Активация аварийного режима
   - Массовая блокировка файлов
   - Уведомление администратора

## Безопасность

- Служба работает только под root
- Хранилище хэшей: права 600
- IPC socket: доступен для чтения всем пользователям
- Резервные копии: права 600
- Логи: права 600

## Лицензия

Бакалаврский проект